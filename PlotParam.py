...
class PlotParam(tkSimpleDialog.Dialog):
    
    def __init__(self, master, limits):
        # override the __init__ of tkSimpleDialog.Dialog
        # limits is a list of the current xmin, xmax, ymin, ymax
        self.plotLimits = limits
    ...
    def body(self, master):
        ...
        Label(master, text='Min Y').grid(row=2)
        Label(master, text='Max Y').grid(row=3)

        self.e1 = Entry(master, justify=CENTER)
        self.e1.grid(row=0, column=1)
        self.e1.insert(END, int(self.plotLimits[0]))

        self.e2 = Entry(master, justify=CENTER)
        self.e2.grid(row=1, column=1)
        self.e2.insert(END, int(self.plotLimits[1]))

        self.e3 = Entry(master, justify=CENTER)
        self.e3.grid(row=2, column=1)
        self.e3.insert(END, int(self.plotLimits[2]))

        self.e4 = Entry(master, justify=CENTER)
        self.e4.grid(row=3, column=1)
        self.e4.insert(END, int(self.plotLimits[3]))
